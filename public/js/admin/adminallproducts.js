...
axios.get('../getAllProductsWithCategories').then(request => {
    console.log(request.data);
    var itemcount = [0,0,0,0,0];
    request.data.forEach(element => {
        var itemcategory = 0;
       if(element['parentCategory'] == 0){
           if(element['category'] >= 1 && element['category'] <= 5) {
               itemcategory = element['category'];
           }
       }
       else{
           if(element['parentCategory'] >= 1 && element['parentCategory'] <= 5){
               itemcategory = element['parentCategory'];
           }

       }

       if(itemcategory && itemcount[itemcategory-1] < 3) {
...
// comments part start ---
           ...
           document.getElementById('partproducts_' + itemcategory).append(mainElement);
           itemcount[itemcategory-1]++;
       }
    });
