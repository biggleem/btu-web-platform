...
function createEditProduct(element){
    let mainproduct = document.createElement('div');
    mainproduct.classList.add('admin_notifications_block_item');
    ...
    block1divimg.addEventListener('click', function () {
        axios.get('rejectProduct/' + element['id']).then(r => {
    ...
    let block1divimgapprove = document.createElement('img');
    block1divimgapprove.classList.add('ok_image');
    block1divimgapprove.src = imagepath + '/ok.png';

    block1divimgapprove.addEventListener('click', function () {
        axios.get('approveProduct/' + element['id']).then(r => {
            if (r.data['status'] == 'success') {
                location.reload();
            } else {
                console.log(r.data);
    ...
    block1div.append(block1divimgapprove);
    ...
    document.getElementsByClassName('admin_notifications_block')[0].append(mainproduct);
}

axios.get('getQueueProducts').then(r => {
    console.log(r.data);
    if(r.data != 'No products') {
        r.data.forEach(element => {
            createEditProduct(element);
        });
    }
});
