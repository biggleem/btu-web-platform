...
// comments part start ---
        ...
        document.getElementsByClassName('section2_items')[0].append(mainElement);
    });

    document.getElementsByClassName('admin_dashboard_banner')[0].addEventListener('click', function(){
    ...
    document.getElementById('dashpicker').addEventListener('change', function(){
        if(this.files.length > 0){
            if (this.files[0].type.match('image.*')) {
                ...
                axios.post('admin/changemainimage', formdata).then(response =>{
    ...
    axios.get('getmainimageext/0').then(r => {
        document.getElementsByClassName('boxed_water_img')[0].src=imagepath+'/mainImages/'+r.data;
    });
